...
window.addEventListener('scroll', function() {
    const header = document.getElementById('header');
    const body = document.body;
    const isSmall = header.classList.contains('small');
    const scrollPoint = 100; // Оптимальная точка

    if (window.scrollY > scrollPoint && !isSmall) {
        header.classList.add('small');
        body.classList.add('small-header');
    } else if (window.scrollY <= scrollPoint && isSmall) {
        header.classList.remove('small');
        body.classList.remove('small-header');
    }
});
